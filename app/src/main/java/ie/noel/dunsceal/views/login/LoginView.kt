...
open class LoginView : BaseView(), View.OnClickListener {
  ...
  // Do not call updateUI in LoginView
  ...
  private fun signOut() {
    if (presenter.auth != null) {
      presenter.auth!!.signOut()
      presenter.app.googleSignInClient.signOut()
      updateUI(null)
    }
  }
  ...
  private fun updateUI(user: FirebaseUser?) {
    ...
    if (user != null) {
      ...
      if (login_detail != null) {
        login_detail.text = getString(R.string.firebase_status_fmt, user.uid)
      }
      ...
      login_status.setText(R.string.signed_out)
  ...
  override fun onClick(v: View) {
    when (v.id) {
      R.id.signUp -> createAccount(fieldEmail.text.toString(), fieldPassword.text.toString())
