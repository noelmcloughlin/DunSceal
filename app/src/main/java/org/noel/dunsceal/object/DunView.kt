...
package org.noel.dunsceal.duns
...
/** Inflates and populates a [View] representing a [Dun]  */
class DunView(layoutInflater: LayoutInflater, container: ViewGroup?) {
    val view: View = layoutInflater.inflate(R.layout.dun2_item_layout, container, false)
    ...
    /**
     * Updates the view to represent the passed in dun
     */
    fun bind(dun: Dun) {
        textSuite.text = dun.suit
        view.setBackgroundResource(getColorRes(dun))

        val cornerLabel = dun.cornerLabel
        textCorner1.text = cornerLabel
        textCorner2.text = cornerLabel
    }

    @ColorRes
    private fun getColorRes(dun: Dun): Int {
        val shade = getShade(dun)
        val color = getColor(dun)
        return COLOR_MAP[color][shade]
    }

    private fun getShade(dun: Dun): Int {
        when (dun.value) {
            "2", "6", "10", "A" -> return 2
            "3", "7", "J" -> return 3
            "4", "8", "Q" -> return 0
        ...
        throw IllegalStateException("Dun value cannot be $dun.value")
    }

    private fun getColor(dun: Dun): Int {
        when (dun.suit) {
            "♣" -> return 0
            "♦" -> return 1
            "♥" -> return 2
        ...
        throw IllegalStateException("Dun suit cannot be $dun.suit")
    }
